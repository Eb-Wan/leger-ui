export class LGUIComponent {
    ...
    constructor(path, parent, root, args = {}) {
        ...
        if (!component) throw new Error(`${this.#instanceOf} : cannot instanciate "${ path }", not such component`);
    ...
    _get(componentPath) {
        ...
        if (!component) throw new Error(`${this.#instanceOf} : not such component "${ componentPath }"`);
