...
async function compile(srcFile, outDir, params = {}) {
    ...
    Object.assign(preprocessorData, params);
    ...
    try {
        const { LGUIComponent } = await import(`${appPath}?t=${Date.now()}`);
        const root = new LGUIComponent("/"+basename(srcFile));
        ...
        return console.error("[ ERROR ] Error while rendering project :", err);
    }
