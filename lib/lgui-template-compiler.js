...
function tokenize(xml) {
    ...
    if (currentMode != text) throw new Error ("Open block or tag");
...
function buildDOM(tokens) {
    ...
    if (currentNode.parent != undefined) throw new Error("Opening tag without corresponding closing tag");
...
function compileDOM(dom) {
    ...
    dom.children.forEach(element => {
        if (element.tagName == "#text") string += element.value;
            ...
            string += `\${${element.tagName}._instantiate({${ args.join()}})}`;
        }
        else if (element.tagName[0].match(/[a-z]/)) {
