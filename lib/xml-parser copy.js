...
function tokenizeXML(xml) {
    ...
    var modes = {
        tag: (char) => {
            if (char == "\\") switchMode(modes.escape);
        ...
        identifier: (char, prevChar) => {
            buffer += prevChar;
            if (!char.match(identifierRegex)) { pushToken("identifier"); switchMode(previousMode); }
        },
        string: (char, prevChar) => {
            buffer += prevChar;
            if (char == "\\") { pushToken("slot"); switchMode(modes.escape); }
            else if (char == "\"") { pushToken("slot"); switchMode(previousMode); }
        },
        number: (char, prevChar) => {
            buffer += prevChar;
            if (!char.match(numberRegex)) { pushToken("slot"); switchMode(modes.previousMode); }
        },
        text: (char, prevChar) => {
            if (char == "\\") { pushToken("text"); switchMode(modes.escape); }
            else if (char == "<") { pushToken("text"); switchMode(modes.tag); }
            else if (char == "{") { pushToken("text"); switchMode(modes.slot); }
        ...
        operator: (char, prevChar) => {
            buffer += prevChar;
            if (!char.match(identifierRegex)) { pushToken("identifier"); switchMode(modes.tag); }
        },
        escape: (char) => {
            buffer += `&#${ char.charCodeAt(0) };`;
            switchMode(modes.previous);
        }
    };

    var currentMode = modes.text;
    var previousMode = modes.text;
    ...
    if (buffer && currentMode != modes.text) throw new Error("Syntax error");
    else if (buffer) pushToken("text");
